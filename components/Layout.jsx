...
const Layout = ({children}) => {
  ...
  const {isOn, setNightColor} = useContext(deviceContext)
  ...
  function changeNightColor(event){
    let selected = event.target.dataset.color
    setNightColor(selected)
        ...
        { !isOn &&
          <motion.div
            ...
            <div className="flex items-center justify-between flex-col h-full">
              <div data-color={"bg-gray-400"} onClick={changeNightColor} className={` w-10 h-10 rounded-full bg-gray-400 hover:scale-[1.1] transition duration-200 ease-in-out cursor-pointer`}/>
              <div data-color={"bg-violet-400"} onClick={changeNightColor} className={` w-10 h-10 rounded-full bg-violet-400 hover:scale-[1.1] transition duration-200 ease-in-out cursor-pointer`} />
              ...
              <div data-color={"bg-purple-600"} onClick={changeNightColor} className={` w-10 h-10 rounded-full bg-purple-600 hover:scale-[1.1] transition duration-200 ease-in-out cursor-pointer`}/>
